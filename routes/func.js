    ...
    this.KBLEfficiency =function (year) {
        var data = {};
        var re = /(\w+)\S(\w+)\S(\w+)/; // 시간 00:00:00 정규표현식
        for (key in teamName) {
            var temp = 0; var sum = 0; var nanCount = 0;
            for (var c = 0; c < (obj[year][key]).length; c++) {
                var str = obj[year][key][1]["Min."];
                var playTime = Number(str.replace(re, "$2")) + Number(str.replace(re, "$1")) * 60;
                temp += ((Number(obj[year][key][c]["2P"]) + Number(obj[year][key][c]["3P"])) + Number(obj[year][key][c]["STL"]) + Number(obj[year][key][c]["BS"]) + Number(obj[year][key][c]["Defensive"])) + ((Number(obj[year][key][c]["Offensive"]) + Number(obj[year][key][c]["Ast"]) + Number(obj[year][key][c]["GD"])) * 1.5 + playTime / 4);
                temp = temp - (Number(obj[year][key][c]["TO"]) * 1.5) + (Number(obj[year][key][c]["2PA"]) - Number(obj[year][key][c]["2P"])) +  (Number(obj[year][key][c]["3PA"]) - Number(obj[year][key][c]["3P"]) * 0.9) + (Number(obj[year][key][c]["FTA"]) - Number(obj[year][key][c]["FT"]) * 0.8) ;
                if(isNaN(temp)) {
                    nanCount++;
                    continue;
                }
